...
const cursorHighlight = (node: Node, left?: Cursor, right?: Cursor, inside: number | null = null): Highlight | undefined => {
    ...
    if (node.type === 'text') {
        const lat = left?.type === 'text' ? getSpanIndex(node.spans, left.end.index) : undefined;
        const rat = right?.type === 'text' ? getSpanIndex(node.spans, right.end.index) : undefined;
        return {
            type: 'text',
...
const innerSide = (outer: SelStart, inner: SelStart, getNode: (id: string) => Node): { side: 'before' | 'after'; at: number } => {
    const node = getNode(lastChild(outer.path));
    ...
    if (outer.cursor.type === 'text') {
        ...
        if (at === outer.cursor.end.index) {
            ...
            const cindex = getSpanIndex(node.spans, outer.cursor.end.index);
            return { side: at < cindex ? 'after' : 'before', at };
        }
    }
...
export const compareSelections = (one: SelStart, two: SelStart, top: Top) => {
    ...
    const side = innerSide(outer, inner, (id) => top.nodes[id]);
    if (side.side === 'before') {
...
const hasNoBraces = (node: Node): node is List<NodeID> => {
    return node.type === 'list' && (node.kind === 'smooshed' || node.kind === 'spaced');
};

export type Neighbor = { path: Path; hl: Highlight };

const getNeighbors = (path: Path, i: number, getNode: (id: string) => Node, side: 'before' | 'after') => {
...
export const getCurrent = (selection: NodeSelection, top: Top): Current => {
    ...
    if (node == null) {
        console.log('NO WHAT', path);
        throw new Error('bad path');
    }
    const cursor = sel.cursor;
    if (node.type === 'id') {
        if (cursor.type !== 'id') {
            throw new Error(`id select must have cursor id, not ${cursor.type}`);
        }
...
export const collectSelectedNodes = (
    ...
    for (let i = 1; i < one.path.children.length && i < two.path.children.length; i++) {
        ...
        const t = two.path.children[i];
        if (o === t) continue;
        const pnode = getNode(one.path.children[i - 1]);
        ...
        if (pnode.type === 'text') {
            ...
            neighbors.push(...getNeighbors(one.path, i, getNode, oat < tat ? 'after' : 'before'));
            neighbors.push(...getNeighbors(two.path, i, getNode, oat < tat ? 'before' : 'after'));
    ...
    const [outer, inner] = one.path.children.length < two.path.children.length ? [one, two] : [two, one];
    const side = innerSide(outer, inner, getNode);
    if (side.side === 'before') {
        neighbors.push(...getNeighbors(inner.path, outer.path.children.length - 1, getNode, 'before'));
        return [outer, neighbors, inner, side];
    } else {
        neighbors.push(...getNeighbors(inner.path, outer.path.children.length - 1, getNode, 'after'));
